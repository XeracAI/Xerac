...
import type { ModelGroupWithModels, ModelGroup } from './types';
...
function injectIconToModelGroups(
  modelGroups: ModelGroupWithModels[],
): ModelGroup[] {
...
function injectModelToModelGroups(
  modelGroups: ModelGroupWithModelIDs[],
): ModelGroupWithModels[] {
  return modelGroups.map((modelGroup) => ({
    ...modelGroup,
    models: modelGroup.models
      .map((modelId) => getAIModelById(modelId))
      .filter((model) => model !== undefined),
  }));
}

const defaultMetaDocument = getMetaDocumentById(
  process.env.DEFAULT_META_DOCUMENT_ID || 'default-models',
);
export const DEFAULT_CHAT_MODEL_ID: string =
  defaultMetaDocument?.defaultModel || 'gemini-2.0-flash-lite-001';
const defaultModelGroups: ModelGroup[] = injectIconToModelGroups(
  injectModelToModelGroups(defaultMetaDocument?.defaultModelGroups || []),
);
 ...
 */
export async function fetchUserModelGroups(
  userId: string,
): Promise<ModelGroup[]> {
