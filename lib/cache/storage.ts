...
class Storage {
  ...
  public async initializeModels(): Promise<void> {
    try {
      ...
      console.log(`Model cache initialized with ${this.models.length} models at ${this.lastRefreshed.toISOString()}`);
    } catch (error) {
  ...
  public async initializeMeta(): Promise<void> {
    try {
      ...
      this.metaMap = new Map(
        this.metaDocuments.map(meta => [meta.id, meta])
      );

      this.metaLastRefreshed = new Date();
      console.log(`Meta cache initialized with ${this.metaDocuments.length} documents at ${this.metaLastRefreshed.toISOString()}`);
    } catch (error) {
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error('Cache has not been initialized. Call initialize() first.');
    }
  }
...
declare global {
  ...
  var modelCache: Storage;
}
let modelCache: Storage;
if (!global.modelCache) {
  global.modelCache = Storage.getInstance();
}
modelCache = global.modelCache;
...
export const getModelFeatures = (modelId: string) => modelCache.getModelFeatures(modelId);
export const getModelTags = (modelId: string) => modelCache.getModelTags(modelId);
export const getModelsLastRefreshedTime = () => modelCache.getModelsLastRefreshedTime();
...
export const getMetaLastRefreshedTime = () => modelCache.getMetaLastRefreshedTime();
